...
#MAE CV
scores = cross_val_score(regr, x_val, y_val,scoring='neg_mean_absolute_error', cv=cv, n_jobs=-1)
# report performance
print('Mean Absolout Error CV and std: %.3f (%.3f)' % (np.mean(scores), np.std(scores)))

#MSE CV
scores2 = cross_val_score(regr, x_val, y_val,scoring='neg_mean_squared_error', cv=cv, n_jobs=-1)
# report performance
print('Mean Square Error CV and std: %.3f (%.3f)' % (np.mean(scores2), np.std(scores2)))

#RMSE CV
scores3 = cross_val_score(regr, x_val, y_val,scoring='neg_root_mean_squared_error', cv=cv, n_jobs=-1)
# report performance
